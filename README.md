...
- 키보드 네비게이션 지원
  - ESC 키로 모달 닫기 시 라우터 상태 관리
  - 모달 닫기 시 이전 페이지로 올바르게 복귀
  - 브라우저 히스토리 관리 최적화
...
### 모달 시스템
- 키보드(ESC)로 모달을 닫을 때 라우터 상태가 올바르게 복구되지 않는 문제
  - 해결 방안: `useRouter`의 `back()` 메서드 대신 `push()`를 사용하여 명시적 라우팅
  - 모달 닫기 시 이전 URL을 저장하고 복원하는 로직 구현 필요
  - 브라우저 히스토리 스택 관리 개선 필요
